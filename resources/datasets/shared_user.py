...
class DatasetSharedUserCollection(Resource):
    ...
    def post(self, user_id, dataset_id):
        ...
        currently_shared_ids = [shared.user_id for shared in SharedDatasetUserModel.query.filter(
            SharedDatasetUserModel.dataset_id == dataset_id).all()]
        ...
        for user_object in data:
            ...
            shared_user_object = SharedDatasetUserModel(user_id=user_object.get('user_id'),
                                                        dataset_id=dataset.dataset_id)
            permission_objects = PIIModel.query.filter(
                (PIIModel.description.in_(user_object.get('permissions', [])))).all()
    ...
    def delete(self, user_id, dataset_id):
        ...
        for user_object in data:
            ...
            if deleted_user_id not in currently_shared_ids:
                abort(400, f'User {deleted_user_id} does not have access to the dataset.')
            
            SharedDatasetUserModel.query.filter(
                (SharedDatasetUserModel.dataset_id == dataset.dataset_id) & (
                        SharedDatasetUserModel.user_id == deleted_user_id)).delete()
