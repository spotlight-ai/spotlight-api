...
class DatasetOwners(Resource):
    ...
    def get(self, user_id , dataset_id):
        dataset = FlatFileDatasetModel.query.filter(FlatFileDatasetModel.dataset_id == dataset_id).first()
        if not dataset:
    ...
    def post(self,user_id, dataset_id):
        try:
            ...
            new_owners_id = [user for user in request.get_json(force=True).get("owners",[]) if user not in existing_owners]
            
            if len(new_owners_id) > 0:
                new_owners = UserModel.query.filter(UserModel.user_id.in_(new_owners_id)).all()
    ...
    def put(self, user_id, dataset_id):
        try:
            ...
            new_owners_id = [user for user in request.get_json(force=True).get("owners",[])]
