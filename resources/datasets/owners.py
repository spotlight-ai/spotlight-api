        ...
        try:
            ...
            db.session.add(
                DatasetActionHistoryModel(
                    ...
                    notes=f"Users with id {', '.join([str(id) for id in new_owners_id])} added as owners to the "
                          f"dataset.",
                )
            )
            db.session.commit()
        ...
        try:
            ...
            db.session.add(
                DatasetActionHistoryModel(
                    ...
                    dataset_id=dataset.dataset_id,
                    action=AuditConstants.DATASET_OWNERS_MODIFIED,
                    notes=f"Users with id {', '.join([str(id) for id in owners_to_be_removed])} removed as owners of "
                          f"the dataset.",
                )
            )
            db.session.commit()
