...
class RoleMemberCollection(Resource):
    ...
    def post(self, user_id, role_id):
        ...
        try:
            ...
            role_member_objects = self.format_role_members(users, current_members, role_id)
            role_member_objects.extend(self.format_role_members(owners, current_members, role_id, is_owner=True))
    ...
    def put(self, user_id, role_id):
        ...
        try:
            ...
            role_member_objects.extend(self.format_role_members(owners, [], role_id, is_owner=True))
    ...
    def delete(self, user_id, role_id):
        ...
        RoleMemberModel.query.filter((RoleMemberModel.role_id == role_id) &
                                     (RoleMemberModel.user_id.in_(all_members))).delete(synchronize_session='fetch')
        role.updated_ts = datetime.datetime.now()
        db.session.commit()
        return
    ...
    def format_role_members(members, current_members, role_id, is_owner=False):
        ...
        for member in members:
            ...
            role_member_objects.append({'role_id': role_id, 'user_id': member, 'is_owner': is_owner})
