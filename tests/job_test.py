...
from tests.conftest import job_route
...
def test_extra_field_job_creation(client, db_session):
    ...
    assert "Unknown field." == str(res.data)
...
def test_missing_field_job_creation(client, db_session):
    ...
    assert res.status_code == 422
    assert "Missing data for required field." == res.data.decode()
...
def test_missing_auth_retrieve_all_jobs(client, db_session):
    ...
    assert AuthenticationErrors.MISSING_AUTH_HEADER == res.data.decode()
