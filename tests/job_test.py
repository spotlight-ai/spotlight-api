...
class JobResourceTest(BaseTest):
    def test_valid_job_creation(self):
        ...
        res = self.client().post(self.job_route, headers=headers, json={'dataset_id': datasets[0].get('dataset_id')})
    ...
    def test_missing_field_job_creation(self):
        ...
        res = self.client().post(self.job_route, headers=headers, json=wrong_job)
