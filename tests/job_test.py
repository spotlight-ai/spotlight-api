...
class JobResourceTest(BaseTest):
    def test_valid_job_creation(self):
        ...
        res = self.client.post(
            self.job_route, headers=headers, json={"dataset_id": 1}
        )
    ...
    def test_extra_field_job_creation(self):
        ...
        res = self.client.post(
            self.job_route, headers=headers, json={"dataset_id": 1, "name": "John"}
        )
    ...
    def test_nonexistent_dataset_job_creation(self):
        ...
        res = self.client.post(
            self.job_route, headers=headers, json={"dataset_id": 9}
        )
    ...
    def test_access_all_active_jobs_all_datasets_user(self):
        ...
        header_user1 = self.generate_auth_headers(self.client, user_id=1)
        header_user3 = self.generate_auth_headers(self.client, user_id=3)
        header_user4 = self.generate_auth_headers(self.client, user_id=4)
    ...
    def test_access_all_active_jobs_all_datasets_admin(self):
        ...
        header_user5 = self.generate_auth_headers(self.client, user_id=5)
        header_user3 = self.generate_auth_headers(self.client, user_id=3)
        header_user4 = self.generate_auth_headers(self.client, user_id=4)
    ...
    def test_change_job_status_user(self):
        """Ensures that users are unable to change job status"""
        header_user3 = self.generate_auth_headers(self.client, user_id=3)

        # User 3 posts a job for a Dataset they own
        self.client.post(
            f"{self.job_route}/1", headers=header_user3, json={"dataset_id": 1}
        )
    ...
    def test_change_job_status_model(self):
        ...
        header_user3 = self.generate_auth_headers(self.client, user_id=3)
        # header_model = self.generate_auth_headers("MODEL_KEY")
