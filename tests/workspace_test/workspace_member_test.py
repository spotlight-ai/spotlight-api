...
def _delete_member(client, auth_user_id, delete_user_id):
    ...
    res = client.delete(f"/workspace/{workspace_id}/member/{delete_user_id}", headers=headers)
    return res
...
def _assert_member_exists_once(user_id):
    len(WorkspaceMemberModel.query.filter(
        WorkspaceMemberModel.workspace_id == workspace_id,
        WorkspaceMemberModel.user_id == user_id,
    ).all()) == 1
...
def test_token_expired(client, db_session):
    ...
    body, headers = _add_member(client, user_to_add, expires_delta=datetime.timedelta(-1, 86392, 895000))
    res = client.post(f"/workspace/{workspace_id}/member", json=body, headers=headers)
